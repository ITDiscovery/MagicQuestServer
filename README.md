...
A self-contained, open-source game engine running entirely on an ESP32 that decodes authentic MagiQuest wands and triggers interactive, branching video quests. 

This project liberates dead, proprietary theme-park tech and allows you to build your own interactive wand-based escape rooms, puzzles, or living room adventures.
...
* **Persistent Player Save States:** Uses LittleFS to remember which wand belongs to which player, and exactly what page of the story they are on.
...
*Note: Wire-wrapping is highly recommended for the XIAO board to avoid lifting pads or melting traces.*

## 🚀 How to Play
1. **Power On:** Plug the stone into any standard USB wall charger.
2. **Wi-Fi Setup:** On first boot, the ESP32 creates an Access Point. Connect to it to pass your home Wi-Fi credentials to the stone. 
3. **Author the Game:** Navigate to the stone's IP address on your local network and append `/admin` to open the Author Panel. Link your YouTube Video IDs to build your story.
